...
// Placeholder imports - migrate later if needed
...
const scoreColor = '#FFD700'; // Gold
const scoreShadowColor = '#c0392b'; // Reddish shadow

// --- Styles (Adapted & Responsive) ---
const styles: Record<string, React.CSSProperties> = {
    sceneRoot: { position: 'absolute', inset: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column', backgroundColor: defaultBgColor },
    backgroundMedia: { position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', zIndex: 0 },
    overlay: { position: 'absolute', inset: 0, zIndex: 1, backgroundColor: 'rgba(0, 0, 0, 0.4)' }, // Default overlay
    // particleLayer: { position: 'absolute', inset: 0, zIndex: 1, pointerEvents: 'none' },
    ...
    scoreText: {
        ...
        fontSize: 'clamp(70px, 20vw, 160px)',
        marginBottom: 'clamp(15px, 5vh, 50px)', color: scoreColor,
        textShadow: `4px 4px 0px ${scoreShadowColor}, 8px 8px 0px #000, 10px 10px 15px rgba(0,0,0,0.6)`, // Slightly adjusted shadow for web scale
        letterSpacing: '-0.02em', whiteSpace: 'pre-line',
        position: 'relative', // Needed for motion transform origin
    },
    callToAction: {
        fontFamily: ctaFontFamily,
        fontSize: 'clamp(24px, 7vw, 60px)', fontWeight: 'bold', color: '#FFFFFF',
        textShadow: '2px 2px 5px rgba(0,0,0,0.8)', whiteSpace: 'pre-line',
        lineHeight: 1.2,
        maxWidth: '90%',
    },
    ctaWordSpan: { display: 'inline-block', marginRight: '0.2em', // Use em for spacing relative to font size
                   whiteSpace: 'nowrap' },
};
...
const scoreVariant = {
    ...
    // Add shake/jiggle on animate completion or hover?
};
...
interface SceneProps extends OutroV1Props { // From quizDataSchema
    sceneId: string; durationInSeconds: number;
    onSceneEnd: () => void; // Result not usually relevant for outro
}

export const OutroSceneV1: React.FC<SceneProps> = ({
    sceneId, scoreText, callToAction, backgroundImageUrl, backgroundVideoUrl,
    ...
    const ctaWords = callToAction.split(/(\s+)/).filter(word => word.trim().length > 0); // Split by space, keep spaces if needed adjust filter

    return (
        <motion.div style={styles.sceneRoot} initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
            {/* Background handled globally by App.tsx or use props here */}
             {backgroundVideoUrl && <motion.video src={backgroundVideoUrl} autoPlay muted loop style={styles.backgroundMedia} initial={{scale: 1.1}} animate={{scale: 1.3, y: -15}} transition={{duration: durationInSeconds, ease:'linear'}}/>}
